...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (!user) return;
    const load = async () => {
      const [invoicesRes, expensesRes, bookingsRes] = await Promise.all([
      ...
      setRecentInvoices(invoices.slice(0, 5) as typeof recentInvoices);
      setLoading(false);
    };
    load();
  }, [user]);
  ...
  const statCards = [
    { label: 'Total Invoiced', value: stats.totalInvoiced, icon: TrendingUp, color: 'bg-green-700', format: 'currency' },
    { label: 'Total Paid', value: stats.totalPaid, icon: CheckCircle, color: 'bg-green-600', format: 'currency' },
    { label: 'Total Expenses', value: stats.totalExpenses, icon: Receipt, color: 'bg-amber-600', format: 'currency' },
    { label: 'Net Income', value: stats.totalPaid - stats.totalExpenses, icon: DollarSign, color: 'bg-teal-600', format: 'currency' },
    { label: 'Pending Invoices', value: stats.pendingInvoices, icon: Clock, color: 'bg-blue-600', format: 'number' },
    { label: 'Overdue', value: stats.overdueInvoices, icon: AlertCircle, color: 'bg-red-600', format: 'number' },
    { label: 'Upcoming Bookings', value: stats.upcomingBookings, icon: Calendar, color: 'bg-violet-600', format: 'number' },
    { label: 'Open Invoices', value: stats.pendingInvoices, icon: FileText, color: 'bg-slate-600', format: 'number' },
  ];
